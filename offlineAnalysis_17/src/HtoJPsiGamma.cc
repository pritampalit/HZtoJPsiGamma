...
//*******************************
/*
namespace HtoJPsiGamma::AnaUtil {
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  T->Branch("zm1",&zm1,"zm1/F");
  T->Branch("zm2",&zm2,"zm2/F");
  T->Branch("zm3",&zm2,"zm3/F");
   ...
   for (Long64_t jentry=0; jentry<nentries; jentry++) {
    ...
    fillHist1D("nPhotons", vec_phoP4.size(), wt);
     ...
     if (vec_phoP4[0].Pt()/(vec_muTP4[0] + vec_muTP4[1] + vec_phoP4[0]).M() > 0.384) { fillHist1D("nPhoCat", 0); fillHist1D("nPhoCatWt", 0, wt);}
